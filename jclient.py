...
class SimBot(sleekxmpp.ClientXMPP):
    ...
    def start(self, event):
        ...
        self['xep_0045'].joinMUC(self.mucroom, self.boundjid.bare)

    def message(self, msg):
        print("Received message '{}'".format(msg))
        if msg.get_type() is not 'chat':
            util.warn("message(): Ignoring message of type '"+msg.get_type()+"'")
            return
        if msg['from'] is not self.boundjid.bare:
            self.msg_queue.put(json.loads(msg.body))

    def groupchat_message(self, msg):
        if msg['to'].bare == self.boundjid.bare:
            return
        print("Got group message '{}'".format(msg))
        if msg.get_type() != 'groupchat':
            util.warn("groupchat_message(): Ignoring message of type '"+ msg.get_type() + "'")
